...
if(file.exists("ValoresIniciais_gamma_t_Lambda_it_T23_w09.R")) {
  source("ValoresIniciais_gamma_t_Lambda_it_T23_w09.R")
} else {
  stop("ERRO CRÍTICO: Arquivo 'ValoresIniciais_gamma_t_Lambda_it_T23_w09.R' não encontrado.")
}
...
code_dynamic_gamma <- nimbleCode({
  ...
  for (j in 2:K) {
    gamma[j, 1] ~ dunif(min = 0, max = (1 - sum(gamma[1:(j-1), 1])))
  }
  
  for(t in 2:n_times){
    # CORREÇÃO: Indexar lim_inf e lim_sup com [t] para evitar redefinição
    lim_inf_gamma[t] <- (gamma[1, t-1] - delta) * step(gamma[1, t-1] - delta)
    lim_sup_gamma[t] <- gamma[1, t-1] + delta
    
    gamma[1, t] ~ dunif(min = lim_inf_gamma[t], max = lim_sup_gamma[t])
...
p2 <- ggplot(df_epsilon, aes(x=Time)) + geom_ribbon(aes(ymin=Lower, ymax=Upper), fill="lightblue", alpha=0.5) +
