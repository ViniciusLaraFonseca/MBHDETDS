...
source("ValoresIniciais_uma_regiao.R")
...
code_foward_only_style <- nimbleCode({
  ...
  for (i in 1:n_regions) {
    ...
    #Update or Online posterior distribution 
    for (t in 1:n_times) {
      lambda[i, t] ~ dgamma(at[i, t], rate = bt[i, t])
...
data_full <- list(Y = Y, E = E, x = x,gamma = gamma_fixed)

# Valores iniciais para duas cadeias
inits1 <- list(lambda = lambda_ini, beta = beta_ini )
inits2 <- list(lambda = matrix(1, n_regions, n_times), beta = rnorm(p_params, 0, 0.5))
inits_list <- list(inits1, inits2)

# Construir e compilar o modelo
model_foward_only <- nimbleModel(code_foward_only_style, constants = constants_full, data = data_full, inits = inits1, check = FALSE)
Cmodel_foward_only <- compileNimble(model_foward_only)
...
conf_foward_only <- configureMCMC(model_foward_only, monitors = c("lambda", "beta", "gamma"))
#conf_foward_only$removeSamplers("gamma")
Rmcmc_foward_only <- buildMCMC(conf_foward_only)

# compilar o modelo, depois compilar o MCMC usando o projeto do modelo compilado
Cmodel_foward_only <- compileNimble(model_foward_only)    # se já tiver, ok; senão recompilar aqui
Cmcmc_foward_only <- compileNimble(Rmcmc_foward_only, project = Cmodel_foward_only)

# Executar o MCMC com 2 cadeias
cat("\n--- Executando MCMC com 2 cadeias (pode demorar alguns minutos) ---\n")
samples <- runMCMC(Cmcmc_foward_only, niter = 50000, nburnin = 0, nchains = 2, inits = inits_list, samplesAsCodaMCMC = TRUE,summary = TRUE)
...
save_traceplot <- function(df, param_name, true_value) {
  ...
  filename <- paste0("traceplot_foward_only_gamma_fixed_i1_", gsub("\\[|, |\\]", "_", param_name), ".png")
